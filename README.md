...
Skor tiap ticker pakai model **`direction_xgboost_v5`** (44 fitur, walk-forward validated), hasilkan keputusan BUY/WATCH/AVOID + Entry/SL/TP:

```bash
python -m engine.predict
```

**Target/horizon diganti 2026-09-13** (dari 5%/-2.5%/10 hari ke **10%/-5%/5 hari**) lewat `scripts/search_swing_target.py` — permintaan eksplisit untuk mencari definisi target yang paling optimal, bukan asumsi angka lama sudah terbaik. Grid 20 konfigurasi (4 pasang target/stop x 5 horizon, metrik lift top-5%/10% teratas atas baseline null masing-masing konfigurasi sendiri, satu-satunya cara adil membandingkan lintas-konfigurasi karena target lebih besar otomatis menggeser baseline null). Pemenang: 10%/5%/5 hari (lift +0.228 vs konfigurasi lama +0.168, peringkat 8 dari 20). Threshold BUY di-retune ulang khusus untuk label baru ini lewat `scripts/tune_v5_new_target_threshold.py` (lihat di bawah) sebelum model diretrain dan di-deploy — bukan sekadar tukar konstanta.

Aturan keputusan (`engine/decision.py`, histori tuning lengkap ada di docstring file itu):
- Entry/SL/TP diturunkan dari `target_pct`/`stop_pct` yang sama dengan definisi label (10% / 5%, R:R 2.0).
- **BUY**: probability ≥ **60%** (`BUY_THRESHOLD`, di-retune ulang untuk label baru — beda dari label lama, kurva precision/profit-factornya naik terus tanpa puncak sampai sampel menipis drastis di atas 75%, jadi 60% dipilih sebagai titik seimbang: precision pooled 71.9% [Wilson LB 68.6%], ~1.9 sinyal BUY/hari lintas ~900 ticker) -- **kecuali saat IHSG sendiri sedang turun** (return 20 hari negatif), di mana ambangnya naik ke **65%** (`IHSG_DECLINE_BUY_THRESHOLD`, warisan dari rasio lama, BELUM divalidasi ulang khusus untuk label baru). **WATCH**: probability ≥ base rate historis (~29%) tapi < ambang BUY yang berlaku hari itu. **AVOID**: di bawah base rate, atau harga ≤ Rp50 (gocap floor, tick-size mendominasi sinyal di harga sekecil itu).
- Precision walk-forward pooled (bukan rata-rata fold) tercatat 71.9% di threshold live 60% (Wilson LB 68.6%, n=762 sinyal BUY) — lihat halaman **Info Model** untuk angka lengkap dan penjelasan kenapa pooled dipakai, bukan rata-rata sederhana antar fold.
...
- **Fitur baru (2026-09-08): kecepatan rally 10-hari** (`ret_10d_pct`, `ret_10d_atr_norm` -- return 10 hari dinormalisasi ATR sendiri, lihat `features/technical.py`'s `compute_rally_speed`). Dipicu pertanyaan pengguna: kalau harga sudah naik sangat cepat sebelum sinyal BUY, apakah model tahu risiko "dibanting" tiba-tiba? `scripts/test_rally_speed_feature.py` menguji dua hal: (1) diagnostik -- di antara sinyal BUY yang SUDAH ada, win rate ternyata FLAT di semua tingkat kecepatan rally (bahkan yang paling ekstrem, ~33x ATR normal dalam 10 hari, tetap menang ~78%) -- catatan penting: semua sinyal BUY memang sudah mensyaratkan rally besar (RSI rata-rata 94-97), jadi ini membandingkan "cepat" vs "sangat cepat", bukan "belum naik" vs "sudah naik"; (2) menambahkan kecepatan rally sebagai FITUR TRAINING tetap menaikkan precision zona BUY (pooled walk-forward 80,0%→81,1%, Wilson LB 77,1%→78,4%). Diadopsi -- di-backfill retroaktif (`scripts/backfill_rally_speed.py`) dan model diretrain.
...
**Satu kombinasi Tervalidasi** ditemukan lewat backtest 5 tahun + grid search sistematis (`scripts/backtest_momentum_screener.py`, `scripts/search_momentum_rules.py`, `scripts/grid_search_momentum_rules.py`): regime **bottoming** + momentum histogram menguat + money flow negatif (distribusi, bukan akumulasi -- kounter-intuitif tapi konsisten: saham yang masih terlihat lemah di permukaan justru punya ruang lebih besar untuk mengejutkan naik) + volume relatif ≥0.8x. Ditambah satu lapisan konfirmasi lagi (`scripts/test_strategy_6_criteria.py`, menguji 6 kriteria yang diusulkan pengguna): harga **≥ Anchored VWAP dari titik terendah 50 hari** ("modal rata-rata smart money" sejak harga menyentuh dasar) -- 5 dari 6 kriteria yang diusulkan lemah/lebih buruk dari acak sendirian dan kombo lengkapnya tidak terpakai (n=12, win rate 25%), tapi AVWAP sebagai lapisan tambahan di atas aturan yang sudah tervalidasi terbukti menaikkan win rate. Win rate **42.4%** vs baseline acak 30.6% (target sama seperti Swing: naik ≥5% sebelum -2.5% dalam 10 hari) -- terbukti lebih baik dari acak secara statistik (Wilson 95% lower bound 38.3%, dari 523 kejadian), tapi jauh di bawah precision Swing/Turnaround. Semua kriteria LAIN di halaman ini (divergence, regime priority lama, RSI pivot distance) tetap murni heuristik yang masuk akal tapi belum terbukti -- dilabeli jelas di aplikasi.

## Rekomendasi Emitten (gabungan lintas-alat)

Halaman yang menyaring & menggabungkan hasil Swing + Turnaround + Momentum Screener sekaligus -- tidak ada perhitungan baru, murni agregasi. Backtest (`scripts/backtest_triple_intersection.py`, memakai `scripts/test_strategy_6_criteria.py`'s `build_dataset()` supaya `close_above_avwap`-nya nyata, bukan hasil hardcode) menunjukkan saham yang lolos ketiganya sekaligus (Swing ≥WATCH, Turnaround POTENSIAL, Momentum Tervalidasi -- versi 5-kriteria dengan AVWAP) punya win rate **43.4%** (n=235, Wilson LB 37.2%) vs Momentum Screener sendirian **42.4%** (n=523, Wilson LB 38.3%) -- titik estimasi naik sedikit, tapi batas bawahnya sendiri praktis flat karena sampel mengecil hampir separuhnya (gerbang AVWAP lebih ketat dari kriteria lama). Dengan catatan metodologi: pengujian gabungan ini menjalankan model Swing/Turnaround yang sudah terlatih dari SELURUH histori ke tanggal masa lalu, jadi anggap sebagai indikasi penguat, bukan bukti seketat walk-forward validation asli.
...
- `test_series_cascade.py` — menguji klaim dari paper Pacific-Basin Finance Journal (arsitektur Seri Lasso→MLP→XGBoost mengungguli model tunggal/paralel untuk prediksi return INDEKS global): apakah ini juga berlaku untuk klasifikasi saham individu IDX kita. **Kalah telak di keempat fold** (precision turun 4-14 poin persentase di semua fold, jumlah sinyal BUY meledak 3-4x tanpa presisi yang sepadan -- tanda kalibrasi probabilitas yang jauh lebih buruk). Tidak diadopsi -- bukti langsung bahwa teknik yang terbukti di level indeks tidak otomatis berlaku untuk saham individu di pasar berkembang.
- `test_strategy_6_criteria.py` — menguji strategi 6-kriteria yang diusulkan pengguna (Struktur Lantai, Jenuh Jual/RSI<45, Akumulasi Senyap, Bahan Bakar/RVOL>1, Modal Bandar/Anchored VWAP, Golden Cross di bawah nol) terhadap dataset 76.442 kejadian historis yang sama. Kombo LENGKAP-nya tidak terpakai (n=12, win rate 25% -- di bawah baseline acak) dan 5 dari 6 kriteria lemah/merugikan sendirian, tapi satu kriteria (Anchored VWAP dari titik terendah 50 hari, sebagai lapisan konfirmasi TAMBAHAN di atas aturan yang sudah tervalidasi, bukan berdiri sendiri) terbukti menaikkan win rate 39.8%→42.4% (Wilson LB 36.7%→38.3%, n=958→523) -- diadopsi ke `features/momentum_screener.py`.
- `test_avwap_model_feature.py` — pertanyaan lanjutan: kalau Anchored VWAP membantu RULE Momentum Screener, apakah dia juga membantu sebagai FITUR TRAINING model Swing/Turnaround (walk-forward, sama seperti `test_macd_zscore_feature.py`)? **Tidak** -- praktis tidak ada perubahan pada kedua model (Swing roc_auc 0.5806→0.5793, Turnaround roc_auc 0.6868→0.6869, precision sedikit lebih rendah di keduanya). Kesimpulannya: kedua model sudah punya cukup fitur berkorelasi (cmf_20, rvol_20, macd_hist, price_vs_vwap20_pct, distance_to_support/resistance_pct) untuk mengekstrak informasi yang sama secara otomatis lewat pohon keputusan -- fitur yang membantu RULE tetap (yang tidak bisa "belajar" sendiri) tidak otomatis membantu model ML yang sudah bisa mengombinasikan sinyal serupa. Tidak diadopsi.
- `test_rally_speed_feature.py` + `backfill_rally_speed.py` — dipicu pertanyaan pengguna soal risiko "saham naik cepat lalu dibanting" pada sinyal BUY Swing. Diagnostik: win rate FLAT di semua tingkat kecepatan rally dalam zona BUY (bahkan yang ekstrem, ~33x ATR normal/10 hari, tetap menang ~78%) -- tapi menambahkan kecepatan rally (`ret_10d_pct`, `ret_10d_atr_norm`) sebagai fitur training tetap menaikkan precision zona BUY (80,0%→81,1%, Wilson LB 77,1%→78,4%). **Diadopsi** ke `direction_xgboost_v5` (lihat "Model Swing" di atas).
